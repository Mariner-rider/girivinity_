...
async def recent_events(limit: int = 50):
    ...
    return {"events": [
        {
            "user_id":    r[0], "ip":        r[1],
            "event_type": r[2], "threat":    r[3],
            "severity":   r[4], "detail":    r[5],
            "blocked":    r[6], "timestamp": str(r[7]),
        }
        for r in rows
    ]}
...
async def set_security_mode(mode: str):
    ...
    if mode not in valid:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid mode. Choose: {valid}",
        )
    from app.core import db
    db.execute(
        "INSERT INTO system_security_mode "
        "(mode, triggered_by, set_at) VALUES (%s, 'admin', NOW())",
        (mode,),
    )
    if mode == "emergency":
        from app.security.session_manager import SessionManager
        count = SessionManager().emergency_invalidate_all(
            reason="admin_manual_emergency"
        )
        return {
            "mode": mode,
...
async def threat_summary():
    ...
    rows = db.fetchall(
        ...
        SELECT threat_type,
               COUNT(*),
               AVG(CASE WHEN blocked THEN 1 ELSE 0 END)
        FROM security_events
